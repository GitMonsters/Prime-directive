...
fn detect_pattern(history: &[IterationRecord]) -> Option<String> {
    ...
    if recent.windows(2).all(|w| w[0].state_hash == w[1].state_hash) {
...
fn main() {
    ...
    for iteration in 0..100 {
        ...
        *energy_map.entry(energy_key).or_insert(0) += 1;
        ...
        if let Some(pattern) = detect_pattern(&history) {
            println!("\n⚠ PATTERN DETECTED at iteration {}: {}", iteration, pattern);
    ...
    let max_energy = history.iter().map(|r| r.energy).fold(f64::NEG_INFINITY, f64::max);
    let min_energy = history.iter().map(|r| r.energy).fold(f64::INFINITY, f64::min);
    ...
    for (i, &(&energy_key, &count)) in sorted_by_count.iter().take(5).enumerate() {
        ...
        println!("    {}: E ≈ {:.1}, visited {} times ({:.1}%)",
            i + 1, energy_val, count, 100.0 * count as f64 / history.len() as f64);
    }
    
    // Declaration sequence
    println!("\nDECLARATION SEQUENCE:");
    let unique_declarations: Vec<_> = history.iter()
        .map(|r| r.declaration.clone())
        .collect::<std::collections::HashSet<_>>()
        .into_iter()
        .collect();
    println!("  Unique declarations: {}", unique_declarations.len());
    println!("  Sequence: {:?}", &history.iter()
    ...
    match final_pattern {
        Some(ref pattern) if pattern.starts_with("FIXED_POINT") => {
            ...
            println!("\nResult: FIXED POINT reached at iteration {}", final_iteration);
            println!("\nInterpretation:");
        ...
        None => {
            ...
            println!("  No fixed point or cycle detected after {} iterations", history.len());
            println!("\nInterpretation:");
    ...
    println!("Average time per iteration: {:.3?}", duration / history.len() as u32);
