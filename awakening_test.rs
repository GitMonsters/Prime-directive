...
impl IsingSystem {
    ...
    fn add_thermal_noise(&mut self, temperature: f64, seed: u64) {
        ...
        println!(
            "    [PERTURBATION] Injecting thermal noise (T = {:.2})",
            temperature
        );
    ...
    fn introduce_external_field(&mut self, strength: f64) {
        println!(
            "    [PERTURBATION] Adding external field (strength = {:.2})",
            strength
        );
...
fn main() {
    ...
    for (perturbation_name, perturbation_type) in perturbations {
        ...
        println!(
            "    Energy after perturbation: {:.4}",
            energy_after_perturbation
        );
        println!(
            "    Δ Energy: {:.4}\n",
        ...
        if detect_fixed_point(&post_perturbation_history, 5) {
            ...
            println!(
                "    → Final: \"{}\"",
                post_perturbation_history.last().unwrap().1
            );
        } else {
    ...
    println!(
        "Fixed point reached at iteration: {:?}",
